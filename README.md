...
1. **电脑端**：Windows 可执行文件（`PalmDeck.exe`：托盘常驻 + 后台桥接 + 自动更新），自带**网页控制台**（配置 / 监测 / 更新 / 日志）
...
iOS App：横屏双手握，三套**硬件皮肤**（v4）：
- **飞机**：总距杆（IDLE/FLY/MAX 止动）+ 脚舵 + 周期变距杆 + 仪表板（姿态球/扭矩/滚转俯仰）+ 按键簇
- **开车**：方向盘（多圈+回正）+ 离合/刹车/油门三踏板 + 序列式档杆 + 转速表仪表盘 + 视角板 + 按键簇
- **手柄**：Xbox 皮肤（双摇杆/十字键/ABXY/LB·RB/LT·RT/视图·菜单）；轴停发、不抢电脑键鼠；可切回自定义组件布局
