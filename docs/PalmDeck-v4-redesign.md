...
2. **iOS App**：唯一手机端。按真实硬件设计的三种控制器皮肤。
...
- Xbox 式布局：ABXY、双摇杆（带回中）、LT/RT 扳机、十字键、LB/RB、视图/菜单
- 作为通用手柄皮肤（`GamepadDeck`）；游戏手柄/菜单场景默认用它，但**不抢电脑键鼠**（服务端停轴）
- 可选切回「自定义组件布局」（`palmdeck_gamepad_custom`）：由组件画布 + `/api/layouts` 同步驱动

### 2.4 App 交互规格

屏幕/状态机、三皮肤逐控件交互、触摸手感（抓取增量 / 回中 / 曲线 / 死区 / 灵敏度）、
