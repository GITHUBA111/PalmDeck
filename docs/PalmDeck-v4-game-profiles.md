...
机制是通用的，但用户要的是「打开就有我的游戏」。建议内置表
（**先给少数几个高置信度的，宁可少也不要编**）：

| 预设名 | 模式 | 轴表 | 依据 |
|---|---|---|---|
| **默认（WARDOGS）** | heli | `hotas` | 现状，`docs/PalmDeck-v3-feature-design.md:1036` |
| **Elite Dangerous** | heli | `z=yaw` 类（fbw 家族） | 社区标准：扭杆偏航、油门独立轴 |
| **DCS / MSFS / X‑Plane** | heli | `hotas` | 常规：`Z=油门`、`Rz=偏航` |
| **赛车（Forza/AC/ETS2）** | drive | 走 Xbox（不吃 profile） | `docs/PalmDeck-v3-feature-design.md:1044`「Remap 只作用于 vJoy」 |

**赛车预设的价值不在轴表**（`drive` 模式本来就不吃 `profile`，走 vgamepad），
而在**死区≈0 + 灵敏度 + 按钮标签**（`Widgets.swift:38–47` 已有语义名可复用）。

⚠️ **需要你提供真实游戏清单再定稿**。上表里除 WARDOGS 外，
其余是社区常规做法，**不是本项目验证过的**。见 §6。

### 3.7 顺手修掉的两个死物（与本方案无关，但同一片区域）
...
| 协议 WS 文本 | **G3 新增** `{"type":"profile"}` + `caps` 加 `profile_select`。老电脑静默忽略（`bridge.py:766` 无 else）⇒ 需 `caps` 门控 |
| 电脑侧 | `hotas.py`（`remap_vjoy` 改查表，约 −25/+35 行）、`palmdeck_config.py`（`axis_profile` 枚举放开 + 新 `axes_presets`）、`bridge.py`（WS 分支 + `caps`）、`web/host.html`（下拉框选项改为动态） |
| App 侧 | `ControllerState.swift`（PKey 加模式后缀 + 迁移）、`Model/GameProfile.swift`（新）、`Layout.swift`（模板 → 预设）、`SettingsView.swift`（新 Section）、`CockpitController.swift`（发 `profile` 消息）、`project.pbxproj`（手动登记新文件） |
...
**电脑侧（Python）**
- 扩 `tests/test_profile_remap.py`（现有 3 条）：
  - 两张内置表**逐轴**与改造前的字面值一致（`hotas`/`fbw` 行为零漂移）——
    这是「重构 vs 改行为」的分界线，必须有。
  - 未知预设回落 `hotas`（保住 `test_unknown_profile_is_hotas` 的语义）。
  - 自定义表非法取值（源不在 `SOURCES`、变换不在枚举）被拒绝，不落到默认。
- 新 `tests/test_game_profiles.py`：
  - `{"type":"profile"}` 已知名 → `HUB.axis_profile` 变、`status` 广播、不重建 vJoy。
  - 未知名 → `ok:false` 且 `HUB.axis_profile` **不变**。
  - `hello.caps` 含 `profile_select`。
- `tests/test_config.py` 扩展：`axes_presets` 往返 + 非法结构被拒。
...
**手工（两次点击的端到端）**
1. 建两个预设（heli/hotas 与 heli/Elite 型），来回切，确认死区/反转/布局一起变。
2. 切到 `drive`，确认赛车死区与 heil **互不影响**（G1 的验收点）。
3. 跑一次 `--open-udp` + 真机，确认切换时**没有**「手机已离线」类 failsafe 误报。
...
- **⚠️ 需要你补的信息**：**你实际在玩的游戏清单**。§3.6 的表里只有 WARDOGS 是
  有据可依的，其余是按社区常规推的。没有这个清单，G4 就是编的，我不会写。
...
| 阶段 | 内容 | 规模 | 依赖 | 可独立验收 |
|---|---|---|---|---|
| **G1** | App 手感参数按模式分离 + 迁移 | **S** | 无 | ✅ 赛车/飞机死区互不干扰 |
| **G2** | `GameProfile` + 预设 UI（布局+手感+轴表名） | **M** | G1 | ✅ 本地切换，轴表名只显示不生效 |
| **G3** | 电脑侧声明式轴表 + WS `profile` 消息 + `caps` 门控 | **M** | G2 | ✅ 从 App 两次点击切换，轴表真的变 |
| **G4** | 内置预设库 | **S** | G3 + **你的游戏清单** | ✅ 打开即有对应预设 |
| **G0** | 顺手删 Capacitor 残骸（84 KB + 118 行） | **XS** | 无 | ✅ App 包不再含 `public/` |

建议顺序 **G0 → G1(new) → G2 → G3 → G4**。
G0 独立成 commit（纯删除，不带任何功能）；G1 独立成 commit，因为它是
「当前就是错的」而不是「新功能」。
