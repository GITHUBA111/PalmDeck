...
**做法**：编辑条上「存为模板」旁边加一个 `⋯` 菜单（三行：恢复默认布局 / 清空画布 / 撤销上一次改动）。

- **不做二次确认**：`clear` / `reset` 各自会压一道撤销槽（`pushUndo`），当场就能后悔；
  而且「放弃」也能一键回到进编辑前的状态。跟设置页一致（那里也只是 `role: .destructive` 红字，没弹窗）。
- **没得撤销就置灰**（`.disabled(!canUndo)`），不是点了没反应。
- 菜单里的撤销 = 回到上一次**整表操作**前（应用模板 / 预设 / 清空 / 恢复默认）。
